...
export const RadioAndCheckbox = ({ size, stacked, type, options }) => {
    ...
    return (
        <div className="sui-form-field" role="radiogroup">
                    ...
                    (
                        <label className={`sui-${type}-image`} htmlFor={option.id} key={index}>
                            ...
                            <span className={mainClass.join(' ')}>
                                <input id={option.id} type="radio" name={option.name} aria-labelledby={option.arialabelledby} defaultChecked={option.defaultChecked} disabled={option.disabled} />
